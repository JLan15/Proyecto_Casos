...
{
    public partial class FormEditarDireccionAsignada : Form
    {
        public FormEditarDireccionAsignada()
        {
            InitializeComponent();
        }
        ...
        private void btnEditar_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                if (int.Parse(txtIdDireccion.Text) > 0)
                {

                    cn.AbrirConeccion();
                    SqlCommand com = new SqlCommand("exec dbo.SP_ActualizarDireccionAsignada '" + int.Parse(txtIdDireccion.Text) + "','" + txtNombreDireccionAsignada.Text + "'", cn.AbrirConeccion());
                    com.ExecuteNonQuery();
                    //cn.Close();
